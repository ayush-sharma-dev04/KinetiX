...
# Live webcam (0). To use a recorded video file, set e.g. VIDEO_SOURCE = "warrior_2_video.mp4"
VIDEO_SOURCE = 0

POSE_NAME = "Warrior II"
POSE_LABEL = "Warrior II"

# Pose-Specific Form Thresholds (Warrior II / Virabhadrasana II)
# Deep lunge with T-shape arms at shoulder height and upright torso:
FRONT_KNEE_MIN = 85.0             # Min front knee angle (deg)
FRONT_KNEE_MAX = 105.0            # Max front knee angle (deg) (standard ~90° cue)
BACK_KNEE_MIN = 160.0             # Min back knee extension angle (deg)
T_SHOULDER_ANGLE_MIN = 75.0       # Min shoulder angle for T-shape arms (deg)
T_SHOULDER_ANGLE_MAX = 105.0      # Max shoulder angle for T-shape arms (deg)
ELBOW_EXTENSION_MIN = 155.0       # Min elbow extension angle (deg)
TORSO_VERTICAL_MAX = 8.0          # Max torso tilt from vertical (deg) (centered, no forward lean)
THIGH_HORIZONTAL_MAX = 20.0       # Max front thigh angle relative to horizontal (deg)
FRONT_KNEE_STACK_MAX = 0.10       # Max front knee overshoot relative to ankle (normalized)
WRIST_LEVEL_DIFF_MAX = 0.10       # Max vertical height difference between wrists (normalized)
...
def extract_features(pose):
    ...
    # Shoulder angles (elbow - shoulder - hip) (crucial T-shape discriminator)
    ...
    # Thigh & arm orientations relative to horizontal
    left_thigh_horiz = horizontal_angle_of_vector(_xy(pose[LEFT_KNEE]) - _xy(pose[LEFT_HIP]))
    right_thigh_horiz = horizontal_angle_of_vector(_xy(pose[RIGHT_KNEE]) - _xy(pose[RIGHT_HIP]))
    features["left_thigh_to_horizontal"] = left_thigh_horiz
    features["right_thigh_to_horizontal"] = right_thigh_horiz

    left_arm_horiz = horizontal_angle_of_vector(_xy(pose[LEFT_WRIST]) - _xy(pose[LEFT_SHOULDER]))
    right_arm_horiz = horizontal_angle_of_vector(_xy(pose[RIGHT_WRIST]) - _xy(pose[RIGHT_SHOULDER]))
    features["left_arm_to_horizontal"] = left_arm_horiz
    features["right_arm_to_horizontal"] = right_arm_horiz
    ...
    # Lead / Front leg identification (leg with acute knee bend)
    if left_knee < right_knee:
        lead_side = "left"
        features["front_knee_angle"] = left_knee
        features["back_knee_angle"] = right_knee
        features["front_hip_angle"] = left_hip
        features["back_hip_angle"] = right_hip
        features["front_thigh_to_horizontal"] = left_thigh_horiz
        features["front_knee_ankle_x_offset"] = float(abs(pose[LEFT_KNEE].x - pose[LEFT_ANKLE].x))
    else:
        lead_side = "right"
        features["front_knee_angle"] = right_knee
        features["back_knee_angle"] = left_knee
        features["front_hip_angle"] = right_hip
        features["back_hip_angle"] = left_hip
        features["front_thigh_to_horizontal"] = right_thigh_horiz
        features["front_knee_ankle_x_offset"] = float(abs(pose[RIGHT_KNEE].x - pose[RIGHT_ANKLE].x))

    features["lead_side"] = lead_side

    # Arm levelness / T-shape verification
    features["wrist_level_diff"] = float(abs(pose[LEFT_WRIST].y - pose[RIGHT_WRIST].y))
    features["wrists_near_shoulder_level"] = (
        abs(pose[LEFT_WRIST].y - pose[LEFT_SHOULDER].y) < 0.18 and
        abs(pose[RIGHT_WRIST].y - pose[RIGHT_SHOULDER].y) < 0.18
    )
...
def is_warrior_2_detected(feat, pose):
    """
    Detects if the user is in Warrior II configuration:
    - Wide lunge stance
    - Asymmetric leg angles (front bent, back straight)
    - Arms in T-shape at shoulder level (shoulder angle ~90°, NOT overhead)
    - Upright torso
    """
    # Stance must be wide
    if feat["stance_to_shoulder_ratio"] < 1.0 and feat["stance_width"] < 0.22:
        return False

    # Leg asymmetry check
    if abs(feat["left_knee_angle"] - feat["right_knee_angle"]) < 20.0:
        return False

    # One knee bent, one knee relatively straight
    if feat["front_knee_angle"] > 130.0 or feat["back_knee_angle"] < 140.0:
        return False

    # T-shape arms check (key differentiator from Warrior I's ~160°-180° overhead reach)
    if not (65.0 <= feat["left_shoulder_angle"] <= 115.0 and 65.0 <= feat["right_shoulder_angle"] <= 115.0):
        return False

    # Upright torso
    if feat["torso_angle"] > 22.0:
        return False

    return True


def evaluate_warrior_2_form(feat):
    """
    Validates Warrior II form against researched biomechanical thresholds.
    Returns (is_correct, feedback_message, failed_rules).
    """
    issues = []

    # 1. Front knee bend depth
    f_knee = feat["front_knee_angle"]
    if f_knee > FRONT_KNEE_MAX:
        issues.append(f"Bend front knee to 90° ({f_knee:.1f}° > {FRONT_KNEE_MAX}°)")
    elif f_knee < FRONT_KNEE_MIN:
        issues.append(f"Front knee bent too deep ({f_knee:.1f}° < {FRONT_KNEE_MIN}°)")

    # 2. Back leg straightness
    b_knee = feat["back_knee_angle"]
    if b_knee < BACK_KNEE_MIN:
        issues.append(f"Straighten back leg fully ({b_knee:.1f}° < {BACK_KNEE_MIN}°)")

    # 3. T-shape arm alignment (arms horizontal at shoulder level)
    if not (T_SHOULDER_ANGLE_MIN <= feat["left_shoulder_angle"] <= T_SHOULDER_ANGLE_MAX and
            T_SHOULDER_ANGLE_MIN <= feat["right_shoulder_angle"] <= T_SHOULDER_ANGLE_MAX):
        issues.append("Extend arms parallel to floor (T-shape)")

    # 4. Elbow extension in T-arms
    if feat["left_elbow_angle"] < ELBOW_EXTENSION_MIN or feat["right_elbow_angle"] < ELBOW_EXTENSION_MIN:
        issues.append(f"Straighten elbows in T-arms ({min(feat['left_elbow_angle'], feat['right_elbow_angle']):.1f}° < {ELBOW_EXTENSION_MIN}°)")

    # 5. Torso upright (no leaning toward front knee)
    if feat["torso_angle"] > TORSO_VERTICAL_MAX:
        issues.append(f"Keep torso centered and upright ({feat['torso_angle']:.1f}° > {TORSO_VERTICAL_MAX}°)")

    # 6. Front thigh horizontal orientation
    if feat.get("front_thigh_to_horizontal", 0) > THIGH_HORIZONTAL_MAX:
        issues.append(f"Lower hips until front thigh is parallel ({feat['front_thigh_to_horizontal']:.1f}° > {THIGH_HORIZONTAL_MAX}°)")

    # 7. Front knee tracking over ankle
    if feat.get("front_knee_ankle_x_offset", 0) > FRONT_KNEE_STACK_MAX:
        issues.append("Stack front knee directly over ankle")

    # 8. Both arms level (no drooping arm)
    if feat.get("wrist_level_diff", 0) > WRIST_LEVEL_DIFF_MAX:
        issues.append("Keep both arms level at shoulder height")

    if not issues:
        return True, "Good Form! Hold Warrior II!", []
...
def draw_hud(frame, status, hold_time, max_hold, feedback, feat_display, visibility, pose_detected):
    ...
    │ FEEDBACK: Good Form! Hold Warrior II!    │
    │                                          │
    │ FRONT KNEE:          92.4°               │
    │ BACK KNEE:          174.1°               │
    │ T-ARMS (SHOULDER):   91.0°               │
    │ TORSO ANGLE:          3.8°               │
    │ VISIBILITY:           0.94               │
    └──────────────────────────────────────────┘
    """
...
def main():
    ...
    front_thigh_smoother = MovingAverage(SMOOTHING_WINDOW)
    ...
    while cap.isOpened():
        ...
        if result.pose_landmarks:
            ...
            if smoothed_visibility >= VISIBILITY_THRESHOLD:
                ...
                feat["front_thigh_to_horizontal"] = front_thigh_smoother.update(raw_feat["front_thigh_to_horizontal"])

                feat_display = {
                    "FRONT KNEE": feat["front_knee_angle"],
                    "BACK KNEE": feat["back_knee_angle"],
                    "T-ARMS (SHOULDER)": feat["avg_shoulder_angle"],
                    "TORSO ANGLE": feat["torso_angle"],
                }

                if is_warrior_2_detected(feat, pose):
                    is_correct, fb_msg, _ = evaluate_warrior_2_form(feat)
                    if is_correct:
                        status = "CORRECT"
                        feedback = fb_msg
                        ...
                        row = {
                            ...
                            "front_hip_angle": round(feat["front_hip_angle"], 2),
                            "back_hip_angle": round(feat["back_hip_angle"], 2),
                            ...
                            "front_knee_ankle_x_offset": round(feat["front_knee_ankle_x_offset"], 4),
                    ...
                    status = "ADJUST FORM"
                    feedback = "Lunge with arms extended in T-shape (Warrior II)"
                    hold_timer.update(False)
            else:
                status = "POSE NOT DETECTED"
