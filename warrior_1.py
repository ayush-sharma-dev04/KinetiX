...
# Live webcam (0). To use a recorded video file, set e.g. VIDEO_SOURCE = "warrior_1_video.mp4"
VIDEO_SOURCE = 0

POSE_NAME = "Warrior I"
POSE_LABEL = "Warrior I"

# Pose-Specific Form Thresholds (Warrior I / Virabhadrasana I)
# Deep lunge with overhead arms and forward-squared hips/torso:
FRONT_KNEE_MIN = 85.0             # Min front knee angle (deg) (avoid over-deep knee collapse)
FRONT_KNEE_MAX = 120.0            # Max front knee angle (deg) (target ~90°-110°)
BACK_KNEE_MIN = 155.0             # Min back knee extension angle (deg)
OVERHEAD_SHOULDER_MIN = 150.0     # Min shoulder angle for arms raised overhead (deg)
ELBOW_EXTENSION_MIN = 150.0       # Min elbow extension angle overhead (deg)
TORSO_VERTICAL_MAX = 15.0         # Max torso tilt from vertical (deg)
FRONT_KNEE_STACK_MAX = 0.12       # Max knee-over-ankle horizontal overshoot (normalized)
SHOULDER_TILT_MAX = 12.0          # Max shoulder tilt from level (deg)
...
def extract_features(pose):
    ...
    # Lead / Front leg identification (leg with more acute knee bend)
    ...
    # Overhead arm check (wrists higher than shoulders in image space)
    features["wrists_above_shoulders"] = (
        pose[LEFT_WRIST].y < pose[LEFT_SHOULDER].y and
...
def is_warrior_1_detected(feat, pose):
    """
    Detects if the user is in Warrior I configuration:
    - Wide sagittal lunge stance
    - Asymmetric leg angles (front bent, back straight)
    - Arms raised overhead (shoulder angle high + wrists above shoulders)
    - Torso upright
    """
    # Stance must be wide
    ...
    # Leg asymmetry check
    if abs(feat["left_knee_angle"] - feat["right_knee_angle"]) < 18.0:
        return False

    # One knee bent, one knee relatively straight
    if feat["front_knee_angle"] > 135.0 or feat["back_knee_angle"] < 135.0:
        return False

    # Arms overhead differentiator (separates Warrior I from Warrior II)
    if feat["avg_shoulder_angle"] < 130.0:
        return False

    if not feat["wrists_above_shoulders"]:
        return False

    # Upright torso
    if feat["torso_angle"] > 30.0:
        return False

    return True


def evaluate_warrior_1_form(feat):
    """
    Validates Warrior I form against researched biomechanical thresholds.
    ...
    if f_knee > FRONT_KNEE_MAX:
        issues.append(f"Bend front knee deeper ({f_knee:.1f}° > {FRONT_KNEE_MAX}°)")
    elif f_knee < FRONT_KNEE_MIN:
        issues.append(f"Front knee too deep ({f_knee:.1f}° < {FRONT_KNEE_MIN}°)")
    ...
    # 3. Overhead arms reach
    if feat["left_shoulder_angle"] < OVERHEAD_SHOULDER_MIN or feat["right_shoulder_angle"] < OVERHEAD_SHOULDER_MIN:
        issues.append(f"Reach arms higher overhead ({min(feat['left_shoulder_angle'], feat['right_shoulder_angle']):.1f}° < {OVERHEAD_SHOULDER_MIN}°)")

    # 4. Overhead elbow extension
    if feat["left_elbow_angle"] < ELBOW_EXTENSION_MIN or feat["right_elbow_angle"] < ELBOW_EXTENSION_MIN:
        issues.append(f"Straighten elbows overhead ({min(feat['left_elbow_angle'], feat['right_elbow_angle']):.1f}° < {ELBOW_EXTENSION_MIN}°)")

    # 5. Torso upright alignment
    if feat["torso_angle"] > TORSO_VERTICAL_MAX:
        issues.append(f"Keep torso upright ({feat['torso_angle']:.1f}° > {TORSO_VERTICAL_MAX}°)")

    # 6. Front knee tracking over ankle
    if feat.get("front_knee_ankle_x_offset", 0) > FRONT_KNEE_STACK_MAX:
        issues.append("Stack front knee directly over ankle")

    # 7. Shoulder level / square forward
    if feat["shoulder_tilt_angle"] > SHOULDER_TILT_MAX:
        issues.append("Square and level shoulders forward")

    if not issues:
        return True, "Good Form! Hold Warrior I!", []
...
def draw_hud(frame, status, hold_time, max_hold, feedback, feat_display, visibility, pose_detected):
    ...
    │ FEEDBACK: Good Form! Hold Warrior I!     │
    │                                          │
    │ FRONT KNEE:     94.2°                    │
    │ BACK KNEE:     172.8°                    │
    │ OVERHEAD ARMS: 168.4°                    │
        ...
        if result.pose_landmarks:
            ...
            if smoothed_visibility >= VISIBILITY_THRESHOLD:
                ...
                feat_display = {
                    ...
                    "OVERHEAD ARMS": feat["avg_shoulder_angle"],
                    "TORSO ANGLE": feat["torso_angle"],
                }

                if is_warrior_1_detected(feat, pose):
                    is_correct, fb_msg, _ = evaluate_warrior_1_form(feat)
                    if is_correct:
                        status = "CORRECT"
                        feedback = fb_msg
                    ...
                    status = "ADJUST FORM"
                    feedback = "Lunge with arms raised overhead (Warrior I)"
                    hold_timer.update(False)
            else:
                status = "POSE NOT DETECTED"
