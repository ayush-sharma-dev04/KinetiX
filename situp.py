...
# Live webcam (0). To use a recorded video file, set e.g. VIDEO_SOURCE = "situp_video.mp4"
VIDEO_SOURCE = 0

# State-detection thresholds (in degrees relative to horizontal)
# Torso angle relative to floor: 0° = lying flat (DOWN), >= 60° = sitting up (UP)
TORSO_DOWN_THRESHOLD = 20.0       # <= this => lying flat on back (DOWN)
TORSO_UP_THRESHOLD = 60.0         # >= this => sitting upright at apex (UP)
HIP_UP_CONFIRM_THRESHOLD = 75.0   # Secondary hip angle confirmation for apex
...
HIP_TILT_THRESHOLD = 15.0        # Excessive pelvic / torso rotation during rise
SHOULDER_TILT_THRESHOLD = 15.0   # Shoulder twisting during flexion
NECK_PULL_THRESHOLD = 30.0       # Excessive head flexion independent of torso (pulling neck)
...
#     timestamp, torso_angle, smoothed_torso_angle, left_hip_angle,
#     right_hip_angle, avg_hip_angle, smoothed_hip_angle, hip_tilt_angle,
#     shoulder_tilt_angle, neck_angle, visibility (+ sliding temporal windows)
# - TARGET / GROUND TRUTH LABEL:
#     state (DOWN, ASCENDING, UP, DESCENDING)
# - FORM QUALITY LABELS:
#     form_status (GOOD, FORM ISSUE), feedback
# - DERIVED SESSION OUTPUT:
#     rep_count
# -----------------------------------------------------------------------------
CSV_PATH = "situp_dataset.csv"
FLUSH_EVERY = 30
CSV_COLUMNS = [
    "timestamp",
    "torso_angle",
...
def calculate_torso_angle_to_horizontal(shoulder_l, shoulder_r, hip_l, hip_r):
    ...
    0° = lying completely flat on the back.
    90° = sitting completely upright.
    Uses shoulder & hip midpoints to remain robust against torso twisting.
    """
...
def classify_state(torso_angle, avg_hip_angle, current_state):
    """
    Direction-aware state classifier for sit-ups based on torso inclination.

    Enforces valid biological progression:
      - From DOWN: Can transition to ASCENDING
      - From ASCENDING: Can transition to UP (apex reached) or back to DOWN (incomplete)
      - From UP: Can transition to DESCENDING
      - From DESCENDING: Can transition to DOWN (completed) or back to UP
    """
    if current_state == "DOWN":
        if torso_angle > TORSO_DOWN_THRESHOLD:
            return "ASCENDING"
        return "DOWN"

    elif current_state == "ASCENDING":
        if torso_angle >= TORSO_UP_THRESHOLD:
            return "UP"
        elif torso_angle <= TORSO_DOWN_THRESHOLD:
            return "DOWN"
        return "ASCENDING"

    elif current_state == "UP":
        if torso_angle < TORSO_UP_THRESHOLD - 5.0:
            return "DESCENDING"
        return "UP"
        ...
        if torso_angle <= TORSO_DOWN_THRESHOLD:
            return "DOWN"
        elif torso_angle >= TORSO_UP_THRESHOLD:
            return "UP"
        return "DESCENDING"
...
class SitUpRepCounter:
    ...
    A shallow / incomplete sit-up (DOWN -> ASCENDING -> DOWN without reaching UP)
    ...
    def process_transition(self, new_state):
        ...
        if new_state == "DOWN":
            if self._reached_up:
                self.rep_count += 1
                print(f"[rep] Rep #{self.rep_count} counted (reached UP apex)")
            else:
                self.shallow_count += 1
                print("[rep] Incomplete sit-up - NOT counted (never reached UP apex)")
            self._reached_up = False

        self.state = new_state


def evaluate_situp_form(
    torso_angle, hip_tilt, shoulder_tilt, neck_angle, current_state
):
    """
    Evaluates sit-up biomechanics and form quality independently of rep counter.
    Returns (form_status, feedback_message) tuple.
    """
    feedback = "Good Form"
    is_good = True

    # 1. Check twisting / asymmetric hip or shoulder rise
    if hip_tilt > HIP_TILT_THRESHOLD or shoulder_tilt > SHOULDER_TILT_THRESHOLD:
        is_good = False
        feedback = "Uneven rise - avoid twisting torso"

    # 2. Check neck pulling / excessive cervical flexion
    elif current_state in ("ASCENDING", "UP") and neck_angle < 45.0:
        is_good = False
        feedback = "Don't pull neck - engage core"
...
STATE_COLORS = {
    ...
    "ASCENDING": (255, 200, 0), # Light Orange
    "UP": (0, 255, 0),          # Green
    "DESCENDING": (255, 150, 0),# Dark Orange
}


def draw_hud(
    frame, state, rep_count, smoothed_torso, smoothed_hip,
...
def main():
    ...
    rep_counter = SitUpRepCounter()

    session_start = time.time()

    print("Automatic sit-up state detection running. Press [q] to quit.")
    ...
    while cap.isOpened():
        ...
        if result.pose_landmarks:
            ...
            if smoothed_visibility >= VISIBILITY_THRESHOLD:
                # 1. Torso angle relative to floor & hip angles
                ...
                raw_state = classify_state(smoothed_torso, smoothed_hip, debounced_state.confirmed_state)
                ...
                # 5. Form evaluation
                form_status, feedback = evaluate_situp_form(
        ...
        cv2.imshow("Sit-up Rep Tracker", frame)
    ...
    print(f"Incomplete sit-ups (not counted): {rep_counter.shallow_count}")
