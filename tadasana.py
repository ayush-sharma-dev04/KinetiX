...
# Live webcam (0). To use a recorded video file, set e.g. VIDEO_SOURCE = "tadasana_video.mp4"
VIDEO_SOURCE = 0

POSE_NAME = "Tadasana"
POSE_LABEL = "Tadasana"

# Pose-Specific Form Thresholds (Tadasana / Mountain Pose)
# Straight neutral standing alignment:
TORSO_VERTICAL_MAX = 6.0          # Max torso deviation from vertical (deg)
KNEE_STRAIGHT_MIN = 168.0         # Min knee extension angle (deg)
HIP_STRAIGHT_MIN = 165.0          # Min hip extension angle (deg)
SHOULDER_TILT_MAX = 6.0           # Max shoulder tilt from horizontal (deg)
HIP_TILT_MAX = 6.0                # Max hip tilt from horizontal (deg)
STANCE_SHOULDER_RATIO_MAX = 1.30  # Stance width relative to shoulder width
VERTICAL_STACK_TOLERANCE = 0.08   # Max horizontal drift between shoulders, hips, and ankles (normalized)
NECK_ANGLE_MAX = 14.0             # Max neck alignment angle from vertical (deg)
...
def extract_features(pose):
    ...
    # Stacking offsets
    features["shoulder_ankle_x_diff"] = float(abs(mid_shoulder[0] - mid_ankle[0]))
    features["hip_ankle_x_diff"] = float(abs(mid_hip[0] - mid_ankle[0]))
...
def is_tadasana_detected(feat, pose):
    """
    Detects if the user is in a general upright standing posture consistent with Tadasana:
    - Vertical standing alignment
    - Hips above knees, knees above ankles
    - Stance is relatively narrow
    """
    if feat["torso_angle"] > 20.0:
        return False

    # Hips above knees, knees above ankles (in image coordinates y increases downwards)
    if not (pose[LEFT_HIP].y < pose[LEFT_KNEE].y < pose[LEFT_ANKLE].y and
            pose[RIGHT_HIP].y < pose[RIGHT_KNEE].y < pose[RIGHT_ANKLE].y):
        return False

    # Knees and hips roughly extended
    if feat["avg_knee_angle"] < 150.0 or feat["avg_hip_angle"] < 145.0:
        return False

    # Stance not extremely wide
    if feat["stance_to_shoulder_ratio"] > 1.8:
        return False

    return True


def evaluate_tadasana_form(feat):
    """
    Validates Tadasana form against researched biomechanical thresholds.
    Returns (is_correct, feedback_message, failed_rules).
    """
    issues = []

    # 1. Torso upright alignment
    if feat["torso_angle"] > TORSO_VERTICAL_MAX:
        issues.append(f"Straighten torso ({feat['torso_angle']:.1f}° > {TORSO_VERTICAL_MAX}°)")

    # 2. Knee extension
    if feat["left_knee_angle"] < KNEE_STRAIGHT_MIN or feat["right_knee_angle"] < KNEE_STRAIGHT_MIN:
        issues.append(f"Straighten knees ({min(feat['left_knee_angle'], feat['right_knee_angle']):.1f}° < {KNEE_STRAIGHT_MIN}°)")

    # 3. Hip extension
    if feat["left_hip_angle"] < HIP_STRAIGHT_MIN or feat["right_hip_angle"] < HIP_STRAIGHT_MIN:
        issues.append(f"Open hips fully ({min(feat['left_hip_angle'], feat['right_hip_angle']):.1f}° < {HIP_STRAIGHT_MIN}°)")

    # 4. Shoulder level
    if feat["shoulder_tilt_angle"] > SHOULDER_TILT_MAX:
        issues.append(f"Level shoulders ({feat['shoulder_tilt_angle']:.1f}° > {SHOULDER_TILT_MAX}°)")

    # 5. Hip level
    if feat["hip_tilt_angle"] > HIP_TILT_MAX:
        issues.append(f"Level hips ({feat['hip_tilt_angle']:.1f}° > {HIP_TILT_MAX}°)")

    # 6. Stance width
    if feat["stance_to_shoulder_ratio"] > STANCE_SHOULDER_RATIO_MAX:
        issues.append("Bring feet closer (hip-width)")

    # 7. Vertical stacking (shoulders/hips/ankles)
    if feat.get("shoulder_ankle_x_diff", 0) > VERTICAL_STACK_TOLERANCE or \
       feat.get("hip_ankle_x_diff", 0) > VERTICAL_STACK_TOLERANCE:
        issues.append("Align shoulders over hips and ankles")

    # 8. Head / neck posture
    if feat["neck_angle"] > NECK_ANGLE_MAX:
        issues.append("Align head over shoulders (chin level)")

    if not issues:
        return True, "Good Form! Hold Mountain Pose!", []
...
def draw_hud(frame, status, hold_time, max_hold, feedback, feat_display, visibility, pose_detected):
    ...
    │ FEEDBACK: Good Form! Hold Mountain Pose! │
    │                                          │
    │ TORSO ANGLE:   2.1°                      │
    │ AVG KNEE:    178.4°                      │
    │ AVG HIP:     176.2°                      │
...
def main():
    ...
    neck_smoother = MovingAverage(SMOOTHING_WINDOW)
    ...
    while cap.isOpened():
        ...
        if result.pose_landmarks:
            ...
            if smoothed_visibility >= VISIBILITY_THRESHOLD:
                ...
                feat["neck_angle"] = neck_smoother.update(raw_feat["neck_angle"])
                ...
                if is_tadasana_detected(feat, pose):
                    is_correct, fb_msg, _ = evaluate_tadasana_form(feat)
                    if is_correct:
                        status = "CORRECT"
                        feedback = fb_msg
                        ...
                        row = {
                            ...
                            "standing_knee_angle": round(feat["avg_knee_angle"], 2),
                            "lifted_knee_angle": np.nan,
                            "standing_hip_angle": round(feat["avg_hip_angle"], 2),
                    ...
                    status = "ADJUST FORM"
                    feedback = "Stand upright in Mountain Pose"
                    hold_timer.update(False)
            else:
                status = "POSE NOT DETECTED"
